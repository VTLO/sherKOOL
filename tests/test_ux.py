...
# Parametrized sites should *not* include AdmireMe.Vip, which is acting as the control
...
def test_nsfw_explicit_selection(sites_obj, nsfwsites):
    ...
    for site in nsfwsites:
        assert site in {site.name: site.information for site in sites_obj}
        assert 'AdmireMe.Vip' not in {site.name: site.information for site in sites_obj}
...
def test_no_usernames_provided(cliargs):
    # When not interactive, providing no usernames should still fail
